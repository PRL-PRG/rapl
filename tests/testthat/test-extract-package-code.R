test_that("testthat is properly handled", {
  ...
  files <- extract_package_code(
    ...
    types="tests", out_dir, wrap_fun=wrapper,
    compute_sloc=TRUE, quiet=FALSE
  )

  expect_equal(
    files$file,
    file.path(out_dir, "tests", c("testthat-drv-test1.R", "testthat-drv-test2.R"))
  )
